...
def grid_search(df, grid):
    """Original grid search for backward compatibility"""
    return grid_search_advanced(df, grid)


def grid_search_advanced(df, grid, use_sig=1, reopen_sec=60, long_only=False,
                         exit_on_flip=True, use_bb_safety=True):
    """Grid search with command line argument support"""
    ...
    for combo in itertools.product(*grid.values()):
        ...
        # Add command line args to params
        full_params = {
            **params,
            'use_sig': use_sig,
        ...
        log.info("Testing %s", full_params)
